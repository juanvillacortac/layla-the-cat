...
func updatePlayerMovement(entry *donburi.Entry, player *components.PlayerData, playerObject *resolv.Object, ecs *ecs.ECS) {
	...
	if input.IsJustJumping() {
		if player.OnGround != nil && wallOnHead == nil {
			...
			factory.CreateParticles(ecs, components.ParticlesJump, px, playerObject.Y, !player.FacingRight)
	...
	// Moving horizontally is done fairly simply; we just check to see if layla solid is in front of us. If so, we move into contact with it
	...
	if check := playerObject.Check(0, checkDistance, "solid", "ramp"); check != nil {

		// So! Firstly, we want to see if we jumped up into layla that we can slide around horizontally to avoid bumping the Player's head.

		// Sliding around a misspaced jump is a small thing that makes jumping a bit more forgiving, and is layla different polished platformers
		...
		// 1) We're jumping up into layla (dy < 0),
		...
		if dy < 0 && check.Cells[0].ContainsTags("solid") && slide != nil && math.Abs(slide.X()) <= 8 {
			...
			// If sliding -fails-, that means the Player is jumping directly onto or into layla, and we need to do more to see if we need to come into
	...
	} else if player.SpeedX > PLAYER_FRICTION || player.SpeedX < -PLAYER_FRICTION {
...
func DrawPlayer(ecs *ecs.ECS, screen *ebiten.Image) {
	tags.Player.Each(ecs.World, func(e *donburi.Entry) {
		...
		x, y := o.X-camera.X, o.Y-camera.Y

		if !player.FacingRight {
			components.PlayerAnimations[player.State].Draw(screen, ganim8.DrawOpts(math.Round(x), math.Round(y), 0, -1, 1, 1, 0))
		} else {
			components.PlayerAnimations[player.State].Draw(screen, ganim8.DrawOpts(math.Round(x), math.Round(y)))
		}
	})
}
