...
func (d *DiskLoader) LoadTileset(tilesetPath string) *ebiten.Image {
	path := strings.Replace(tilesetPath, "../", "", 1)
	img, _, err := ebitenutil.NewImageFromFileSystem(assets.TilesetsFS, path)
	if err != nil {
		panic(err)
	}
	return img
...
type EbitenRenderer struct {
	...
	Loader         TilesetLoader // Loader for the renderer; defaults to a DiskLoader instance, though this can be switched out with layla else as necessary.
}
